        ...
        if updates:  # проверка, были ли обновления
            for element in updates:
                ...
                if action_code == 4:  # цифра 4 - это новое сообщение
                    ...
                    # учим программу отличать исходящие сообщения от входящих:
                    flags = []
                    flag = element[2]
                    ...
                    if 2 not in flags:  # если это не исходящее сообщение, то:
                        ...
                        # Если юзер не в сети, то его id начинается с минуса - берём модуль на всякий случай!
                            ...
                            if text.lower() == 'начать':
                                ...
                                send_message(user_id=user_id,
                                             message=f"Рад Вас приветствовать, {get_person_info(person_id=user_id)[0]}!"
                                                     f"\n\nСейчас я постараюсь подобрать Вам подходящую пару."
                                                     f"\n\nЯ пока только учусь, поэтому мне очень важно, "
                                                     f"чтобы Вы правильно отвечали на мои вопросы."
                                                     f"\n\nВведите 'да', если Вы готовы перейти к первому кандидату.")
                                ...
                                send_message(user_id=user_id,
                                             message=f"Если хотите продолжить, вводите 'да'.\n\n"
                                                     f"Если Вы уже подобрали себе подходящую пару - вводите 'нет'.")

                            elif text.lower() == 'нет':
                                send_message(user_id=user_id, message=f"Рад был Вам помочь! Надеюсь, я был полезен! :)"
                                                                      f"\n\nЕсли хотите повторить - введите 'начать'!")

                            else:
                                # Работа с БД, чтобы бот не реагировал на сообщения, запрашивающие у юзера доп. информацию
                                if not select_users(user_id=user_id):  # отправляем подсказку новому юзеру только 1 раз!
                                    send_message(user_id=user_id,
                                                 message=f"Введите \'начать\', чтобы я мог начать поиск партнера.")
