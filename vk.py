...
def send_photo(user_id: int, candidate_id: int, photo_id: int,
    ...
    """ Функция, отправляющая написавшему юзеру фото потенциальных кандидатов. """
                    ...
                    if 2 not in flags:
                        ...
                        if text.isdigit():
                            if 18 <= int(text) <= 65:
                                ...
                                send_message(user_id=user_id,
                                             message=f"Ваш возраст должен быть не меньше 18 и не больше 65 лет, "
                                                     f"иначе я не смогу Вам помочь.\n"
                                                     f"Пожалуйста, повторите ввод, если Вы неверно указали возраст.")
...
def ask_city(user_id: int, token: str = access_token, version: str = api_version) -> int:
    """ Функция, которая запрашивает город юзера и ищет его идентификатор в базе. """

    server, key, ts = call_server()

    send_message(user_id=user_id, message=f"Введите, пожалуйста, название Вашего города.\n"
                                          f"Внимание! Сервис работает только в пределах РФ!")
                ...
                if action_code == 4:
                    ...
                    if 2 not in flags:
                        ...
                        try:
                            ...
                            if not list_cities:  # []
                                send_message(user_id=user_id,
                                             message=f"К сожалению, я не смог отыскать город с таким названием.\n"
                                                     f"Возможно Вы ошиблись при вводе или ищете город за пределами РФ.\n"
                                                     f"Попробуйте снова.")
...
def search_candidates(user_id: int, token: str = access_token, version: str = api_version) -> int:
    """
    Функция, которая по указанным критериям ищет подходящего кандидата для пользователя.
    При этом проверяет, чтобы профиль у кандидата был не закрытым, а также, чтобы он не был в списке уже просмотренных.
...
def get_candidate_photo_id(candidate_id: int, token: str = access_token, version: str = api_version) -> list:
    """
    Функция, которая по id потенциального кандидата получает всю информацию по фотографиям профиля данного кандидата,
    в том числе количество лайков, комментариев, репостов...
    Обрабатывает полученную информацию и возвращает список из id наиболее популярных фотографий конкретного кандидата.
    """
    ...
    try:
        all_photos: list = response['response']['items']  # список словарей (каждый словарь - информация об одной фотографии)

        all_photos_id: list = [item['id'] for item in all_photos]  # список из id всех фотографий кандидата

        if 1 <= len(all_photos_id) <= 3:
            best_photos_id: list = all_photos_id  # отсылаем пользователю все фотографии, которые есть у потенциального кандидата

        else:  # т.к. мы изначально запрашиваем кандидатов только с фотографиями профиля, это значит их больше трёх !!!
            best_photo_dict = {}
